...
- [x] Write remote-script regression tests by executing the generated Python
...
- [x] Run tests and observe failures for reset counters and missing suspension.
- [x] Preserve existing traffic counters during upsert; initialize only inserts.
  Preserve existing non-policy fields (flow/subId/etc.) when editing a client.
- [x] Add `build_vpn_client_suspend_command(worker, access_key)` and
...
- [x] Test SSH failure/invalid confirmation -> pending_suspend, successful disable
  -> suspended; preserve UUID/URI and revoke metadata.
- [x] Run focused tests and Ruff. Commit only task-owned files.
...
- [x] Write API tests using real ASGI/SQLite, patching only SSH or lifecycle remote
...
- [x] Cover notes-only no-op, limit updates, unlimited expiry, shortening, future
...
- [x] Implement focused `stage_subscription_update(db, subscription, updates)`.
...
- [x] Define a reusable `vpn_mutation_lock` in a focused module or policy service.
...
- [x] Extend archive/decommission handling for suspended/pending_suspend keys;
  their permanent revoke semantics must not change.
- [x] Run API/policy/archive/decommission tests and commit verified integration.
...
- [x] Add failing tests for automatic expiry -> suspended, explicit disable ->
  suspended, cancellation -> revoked, resume at future start, renewal while
  pending_suspend, and manual revoke never restored.
- [x] Add suspension counters to lifecycle results. Select due suspensions and
  permanent revocations before syncs, with bounded/fair pending retries.
- [x] Re-evaluate current customer/subscription state before each operation.
...
- [x] Lock conflicting admin transitions against lifecycle. Preserve existing
  per-key commits, cancellation behavior, sanitization and safe-node checks.
- [x] Test timeout/unavailable node leaves correct pending state, recovery applies
  the latest policy, and archive/manual revoke cannot race a stale resume.
- [x] Run all backend tests plus Ruff and review state-machine invariants.
...
- [x] Add suspension response counters and Russian state labels. Make manual
...
- [x] On edit/renew/save reload authoritative key data and optionally request
...
- [x] Replace misleading suspend/revoke copy with reversible-pause explanations.
...
- [x] Spec review against approved design; fix all material discrepancies.
- [x] Code-quality/security review after spec review; fix and re-run verification.
- [x] Update `docs/current-state.md` and `docs/vpn-service.md` with state semantics,
  historical-key compatibility, preserved traffic and retry behavior.
- [x] Full backend suite, Ruff, frontend tests/build, `git diff --check`.
- [x] Merge verified branch into main using existing user authorization, push and
...
- [x] Use a new temporary test customer/subscription/key, not test1, to verify
