    ...
    return _bash(
        "\n".join(
            [
                ...
                "",
                "def read_db_state():",
                "    state = {'settings': {}, 'username': '', 'password': '', 'api_token': '', 'inbound_id': '', 'diagnostics': [], 'rows': ''}",
                "    for path in db_paths:",
                "        if not os.path.exists(path):",
                "            continue",
                "        try:",
                ...
                "                emit('INBOUND_CREATE_STATUS', f'list_http_{list_response.status}: {read_response(list_response)}')",
                ...
                "            else:",
                "                current_step = 'login'",
                "                login_response = request_json(opener, local_url.rstrip('/') + '/login', {'username': state['username'], 'password': state['password']})",
                "                emit('INBOUND_CREATE_STATUS', f'login_http_{login_response.status}: {read_response(login_response)}')",
                "                current_step = 'list_with_session'",
                "                list_response = request_json(opener, local_url.rstrip('/') + '/panel/api/inbounds/list', None, method='GET')",
                "                emit('INBOUND_CREATE_STATUS', f'list_http_{list_response.status}: {read_response(list_response)}')",
                "                current_step = 'add_with_session'",
                "                add_response = request_json(opener, local_url.rstrip('/') + '/panel/api/inbounds/add', payload)",
